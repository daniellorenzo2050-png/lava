...
void compile_to_file(const char *source_filename, const char *output_filename) {
    ...
    printf("[lavac v%s] Lendo arquivo fonte: '%s'...\n", LAVA_VERSION, source_filename);
    ...
    printf("[lavac v%s] Sucesso! Bytecode compilado em: '%s'\n", LAVA_VERSION, output_filename);
}
...
void execute_bytecode_file(const char *filename) {
    ...
    printf("[lavac v%s] Executando '%s' na LavaVM...\n", LAVA_VERSION, filename);
...
void instalar_ambiente_lavac() {
    ...
    if (home) {
        ...
        if (bashrc) {
            fprintf(bashrc, "# Atalho para a Linguagem Lava v%s\n", LAVA_VERSION);
            fprintf(bashrc, "alias lavac='%s/lavac'\n", cwd);
            fclose(bashrc);
        }
    }

    printf("[Auto-Instalação v%s] Ambiente configurado com sucesso para a Lava!\n", LAVA_VERSION);
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 4 && strcmp(argv[2], "-o") == 0) {
        ...
        fprintf(stderr, "[Erro de Sintaxe] Comando inválido para a Lava v%s.\n", LAVA_VERSION);
        printf("Uso correto:\n");
